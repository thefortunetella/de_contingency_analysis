...
function read_edgelist(s::String)
    ...
    while !eof(f)
        line = readline(f)
        numbers = [parse(Int64, m.match) for m in eachmatch(r"\d+", line)]  # Coleta e converte os números para Int64
        if length(numbers) >= 2
...
function cfb_de(g::Graph,
    ...
    # A última retorna os impactos normalizados da população
    ve, ive, global_norms_df, edge_norms_df, local_norms = de_iter!(g,
        edge_index_pop,
        ref_cfb,
        crossover_rate,
        beta_min,
    ...
    # Convertendo DataFrames para Vector{Float64}
